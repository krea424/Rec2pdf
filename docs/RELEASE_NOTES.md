...
- Il Dockerfile del backend copia ora direttamente i manifest e il codice dal monorepo senza richiedere estensioni BuildKit, installa le dipendenze in modalità production e include il bundle `publish-bundle/` prima di esportare `PUBLISH_BUNDLE_ROOT=/app/publish-bundle`, assicurando che `publish.sh` sia disponibile anche sui deploy Cloud Run.【F:rec2pdf-backend/Dockerfile†L1-L39】【F:rec2pdf-backend/server.js†L114-L170】
