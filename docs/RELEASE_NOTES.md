...
- La risoluzione dei percorsi `Scripts/`, `Templates/` e `assets/` privilegia ora variabili d'ambiente (`PUBLISH_SCRIPT`, `PUBLISH_BUNDLE_ROOT`, `PROJECT_ROOT`) e include come default il bundle versionato in `rec2pdf-backend/publish-bundle/`, fornendo log dei percorsi provati quando i file non sono disponibili così da consentire il caricamento manuale degli asset in IDE o durante il deploy.【F:rec2pdf-backend/server.js†L58-L155】【F:rec2pdf-backend/server.js†L204-L240】
- Il Dockerfile del backend copia automaticamente `publish-bundle/` nel layer applicativo e definisce `PUBLISH_BUNDLE_ROOT=/app/publish-bundle`, garantendo che Cloud Run disponga subito dello `publish.sh` professionale senza dover montare manualmente il bundle durante i deploy automatizzati.【F:rec2pdf-backend/Dockerfile†L1-L37】【F:rec2pdf-backend/server.js†L101-L151】
