...
## Pattern bimodali
- **Toggle coerente**: la `ModeSegmentedControl` in header e la command palette condividono i flag `MODE_BASE`/`MODE_ADVANCED`, salvando la scelta in `localStorage` e sincronizzandola con Supabase quando disponibile. Ogni nuova modalità deve usare gli stessi flag per garantire fallback consistenti.【F:rec2pdf-frontend/src/components/layout/AppShell.jsx†L56-L107】【F:rec2pdf-frontend/src/components/CommandPalette.jsx†L59-L190】【F:rec2pdf-frontend/src/context/ModeContext.tsx†L21-L258】
- **Parità feature**: il pannello base (`BaseHome`, `UploadCard`, `PipelinePanel`) e la dashboard avanzata devono offrire le stesse azioni core (record, upload, publish, download) anche se distribuite su componenti differenti; evita di introdurre feature esclusive in advanced senza prevedere alternative base.【F:rec2pdf-frontend/src/features/base/BaseHome.jsx†L10-L108】【F:rec2pdf-frontend/src/features/base/UploadCard.jsx†L6-L138】【F:rec2pdf-frontend/src/features/base/PipelinePanel.jsx†L15-L191】【F:rec2pdf-frontend/src/features/advanced/AdvancedDashboard.tsx†L1-L209】
- **Esperienza boardroom**: quando estendi la modalità advanced, riutilizza le superfici, i token e le animazioni definite in `Create.jsx` per mantenere consistenza con il tema executive; il base mode resta la reference per semplicità e deve continuare a funzionare anche quando i flag avanzati sono disabilitati.【F:rec2pdf-frontend/src/pages/Create.jsx†L65-L210】
...
- Usa i flag `MODE_BASE`/`MODE_ADVANCED` per controllare l’accesso ai moduli: se `VITE_DEFAULT_MODE_FLAGS` non è valorizzato il frontend ricade automaticamente su `MODE_BASE,MODE_ADVANCED` per non nascondere il toggle; su ambienti dove vuoi limitarlo imposta esplicitamente solo `MODE_BASE` e lascia che Supabase aggiunga `MODE_ADVANCED` agli utenti abilitati.【F:rec2pdf-frontend/.env.example†L1-L4】【F:rec2pdf-frontend/src/context/ModeContext.tsx†L24-L205】
