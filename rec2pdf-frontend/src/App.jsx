...
export default function Rec2PdfApp(){
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      localStorage.setItem(PROMPT_SELECTION_KEY, JSON.stringify(promptSelection));
    } catch {
      // ignore persistence errors
    }
  }, [promptSelection]);
  ...
  useEffect(() => {
    if (!promptSelection.promptId) return;
    const exists = prompts.some((prompt) => prompt.id === promptSelection.promptId);
    if (!exists) {
  ...
  const processViaBackend=async(customBlob)=>{
    ...
    try{
      ...
      if (promptSelection.promptId) {
        fd.append('promptId', promptSelection.promptId);
        if (promptSelection.focus && promptSelection.focus.trim()) {
          fd.append('promptFocus', promptSelection.focus.trim());
        }
        if (promptSelection.notes && promptSelection.notes.trim()) {
          fd.append('promptNotes', promptSelection.notes.trim());
        }
      ...
      if(data?.pdfPath){
        ...
        const fallbackPrompt = promptSelection.promptId
          ? {
              ...
              focus: promptSelection.focus || '',
              notes: promptSelection.notes || '',
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
          <div className={classNames("md:col-span-2 rounded-2xl p-6 shadow-lg border", themes[theme].card)}>
            ...
            <PromptLibrary
              ...
              focusValue={promptSelection.focus}
              onFocusChange={handlePromptFocusChange}
              notesValue={promptSelection.notes}
              onNotesChange={handlePromptNotesChange}
              cueProgress={promptSelection.cueProgress || {}}
